...
from src.models.schemas import PlatformState, RiskLevel
...
def _loans(rec) -> None:
    ...
    if not rec.alternative_loan_suggestions:
        info_box("No alternative loan suggestions available.", "info")
        return
    ...
    for loan in rec.alternative_loan_suggestions:
        ...
        with c1:
            st.markdown(
                kpi_card("Loan Amount", loan.amount, "", accent=C["blue"], icon="💰", icon_bg=C["blue_dim"]),
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                kpi_card("Interest Rate", loan.interest_rate_range, "estimated range",
