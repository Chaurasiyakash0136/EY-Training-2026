# -*- coding: utf-8 -*-
"""Colab1_CrewAI_Groq_Pipeline (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tvan3D5ePpuduGPfGyGiPudlhBkH2eqH
...
!pip install "crewai[litellm]" crewai-tools langchain-groq sentence-transformers fastembed -q
...
print("  fastembed             - 100% local embedder, no API key needed")
...
# Cell 5 - Agent 1: Disruption Monitor (FIXED: max_iter=2 + use_system_prompt)
...
# Cell 6 - Agent 2: Route Optimiser (FIXED: max_iter=2 + use_system_prompt)
...
# Cell 8 - Agent 4: Compliance Officer (FIXED: max_iter=2 + use_system_prompt)
...
try:
    del globalflow_crew
    gc.collect()
    print("[OK] Old crew object cleared from memory")
except NameError:
    pass
...
globalflow_crew = Crew(
    ...
    verbose=True,
    memory=False,   # FINAL ANSWER: memory + Groq incompatible in crewai v1.14.3
    output_log_file="crew_run.log",
)

assert globalflow_crew.process.value == "sequential"
assert globalflow_crew.memory == False

print("[OK] GlobalFlow Crew assembled — READY TO RUN")
...
# Extension 1 - Financial Analyst (FINAL WORKING VERSION)
from crewai import Agent, Task, Crew, Process
import gc
...
print("[OK] Task 6: Financial exposure   (context: task_monitor, task_route)")
...
print()
print("[OK] Crew rebuilt with Financial Analyst — READY TO RUN")
print(f"  Agents:  {len(globalflow_crew.agents)}")
print(f"  Tasks:   {len(globalflow_crew.tasks)}")
print(f"  Process: {globalflow_crew.process.value}")
print(f"  Memory:  {globalflow_crew.memory}")
print()
print("Now re-run Cell 16.")