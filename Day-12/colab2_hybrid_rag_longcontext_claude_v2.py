# -*- coding: utf-8 -*-
"""Colab2_Hybrid_RAG_LongContext_Claude_v2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Obe6fSap-qQ6DobwUK8mpyNGzluRkThv
...
# ----- Groq -----
GROQ_API_KEY = "Paste Your APi"
os.environ["GROQ_API_KEY"] = GROQ_API_KEY

# ----- Azure AI Search -----
AZURE_SEARCH_ENDPOINT = "https://eygenaiservice.search.windows.net"
AZURE_SEARCH_API_KEY = "Paste Your APi"
AZURE_SEARCH_INDEX_NAME = "rag-colab2"
...
# Keep trainer cost variables if later code expects them
COST_INPUT = 0
COST_OUTPUT = 0
...
CLAUDE_MODEL = "claude-sonnet-3-5-20240620" # Default model, can be overridden
...
def rag_answer(query: str) -> RAGResult:
    """Full instrumented RAG pipeline: embed -> retrieve -> generate."""
    ...
    # Stage 3: Generate (Groq)
    messages = [
        SystemMessage(
            content="Answer ONLY from the provided context. If unsure, say so. Be concise."
        ),
        HumanMessage(
            content=f"<context>\n{context}\n</context>\n\n{query}"
        ),
...
print(
    f"Latency: embed={test.embed_ms:.0f}ms | retrieve={test.retrieve_ms:.0f}ms | generate={test.generate_ms:.0f}ms"
)
...
# Removed direct anthropic client as we are using Groq

def rag_stream(query: str, top_k: int = TOP_K) -> None:
    """Retrieve context then stream LLM response token-by-token."""
    # Retrieve
    ...
    messages = [
        SystemMessage(content="Answer ONLY from the provided context. If unsure, say so. Be concise."),
        HumanMessage(content=f"<context>\n{context}\n</context>\n\n{query}"),
    ]

    # Use the pre-initialized llm (ChatGroq) for streaming
...
# Homework: Why Does Groq Generation Vary So Much for Different Queries?

## Observation

During testing, Groq response generation time varied significantly across different user queries. Some queries were answered almost instantly, while others took noticeably longer.

## Analysis

The variation is mainly caused by differences in the amount of work the Large Language Model (LLM) must perform for each query.

### 1. Input Context Size

Different queries retrieve different amounts of context from the vector database.

* Short context → fewer input tokens → faster processing.
* Large context → more input tokens → slower processing.

Since LLMs process tokens, larger prompts naturally increase latency.

### 2. Output Length

Some questions require very short answers, while others require detailed explanations.

Example:

* "What is FAISS?" → short response.
* "Explain how hybrid search combines BM25 and vector retrieval." → longer response.

More generated tokens increase response time.

### 3. Query Complexity

Simple factual questions require minimal reasoning.

Complex questions involving comparisons, multi-step reasoning, or technical explanations require additional computation by the model.

### 4. Retrieval Quality

If the retriever returns highly relevant documents, the model can answer quickly and confidently.

If retrieved context is noisy or less relevant, the model spends more effort generating a coherent answer.

### 5. End-to-End Pipeline Components
...
Generation usually contributes the largest portion of latency, especially for longer responses.

## Conclusion

Groq hardware provides extremely fast inference, but generation latency still varies because different queries require different amounts of context, reasoning, and output tokens. Therefore, response time is primarily influenced by prompt size, retrieval results, query complexity, and answer length rather than the Groq infrastructure itself.
"""