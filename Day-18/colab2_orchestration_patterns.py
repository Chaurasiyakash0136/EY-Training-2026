...
Original file is located at
    https://colab.research.google.com/drive/1A8Mx1Osa6ukzPk-jtv9NZ0vHfvVRJO-V
...
# ============================================================
# Extension 1 · Custom selector_func — hybrid control
# Forces planner first, then lets the LLM route freely
# ============================================================
...
# Custom selector: forces planner on turn 1, defers to LLM after
def force_planner_first(messages):
    # Return an agent name (str) to force a speaker, or None to let the model decide.
    if len(messages) <= 1:
        return "planner"
...
# Fresh agents — avoids state bleed from earlier cells
planner, researcher, writer = make_specialists()
...
# ============================================================
# Extension 3 · FIXED — Nested team via SocietyOfMindAgent
# AutoGen 0.7.5: GraphFlow only accepts ChatAgent instances.
# SocietyOfMindAgent wraps a team → exposes it as a ChatAgent.
# Flow: planner → SocietyOfMindAgent(researcher+fact_checker) → writer
# ============================================================
...
# New specialist — student-defined (trainer left this open)
fact_checker = AssistantAgent(
    name="fact_checker",
    model_client=model_client,
    description="Verifies the researcher's bullets for accuracy and flags anything uncertain.",
    system_message=(
        "You receive research bullets from the researcher. "
        "For each bullet mark it ✅ if solid or ⚠️ if uncertain. "
        "Be concise. Do NOT rewrite the content."
    ),
)

# Step 1: Inner RoundRobin team — researcher + fact_checker
research_team = RoundRobinGroupChat(
    [researcher, fact_checker],
    termination_condition=MaxMessageTermination(4),  # 2 full rounds max
)

# Step 2: Wrap team as a ChatAgent so GraphFlow accepts it as a node
# SocietyOfMindAgent runs the inner team to completion,
# then synthesises a single response to pass to the next node
research_node = SocietyOfMindAgent(
    name="research_node",
    team=research_team,
    model_client=model_client,
    description="Runs a researcher + fact-checker team and returns verified research bullets.",
)

# Step 3: Build the outer GraphFlow — research_node is ONE node to the graph
builder = DiGraphBuilder()
builder.add_node(planner).add_node(research_node).add_node(writer)
builder.add_edge(planner, research_node).add_edge(research_node, writer)
